...
from omid_utils.standard import standard_persian, persian_num_to_english
...
def parse_iets_date(text):
    try:
        sp = text.split('-')
        if len(sp) == 2:
            text = sp[0]
            sp = text.split('/')
            day = persian_num_to_english(sp[0].strip())
            month = persian_num_to_english(sp[1].strip())
            year = persian_num_to_english(sp[2].strip())
            return Persian(year, month, day).gregorian_datetime()
        return None
    except:
        return None
...
def get_next_date(regex):
    from datetime import timedelta, datetime
    import re
    now = datetime.now()
    for i in range(1, 366):
        dt = now + timedelta(days=i)
        if re.match(regex, str(dt.date())):
            return dt.date()
    return None
